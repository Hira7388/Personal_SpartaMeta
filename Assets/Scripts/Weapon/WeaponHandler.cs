...
public class WeaponHandler : MonoBehaviour
{
    ...
    [SerializeField] protected float weaponSize; // 무기의 크기
    public float WeaponSize { get => weaponSize; set => weaponSize = value; }

    [SerializeField] protected float attackPower; // 무기의 데미지(탄의 데미지는 따로)
    ...
    // 컴포넌트 가져오기
    private BaseController entityController; // 무기를 장착하는 엔티티 가져오기
    private Animator animator; // 자식 오브젝트에 있다
    private SpriteRenderer weaponRenderer; // 자식 오브젝트에 무기 렌더러

    protected virtual void Awake()
    {
        entityController = GetComponent<BaseController>();
